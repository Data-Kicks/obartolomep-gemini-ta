...
def create_player_aggregated_view(query_type: str = "polars") -> None:
    ...
    if query_type == "polars":
        ...
        teams = conn.execute("""SELECT team_id, name AS team_name FROM dim_teams""").pl()
        players = (conn.execute("""SELECT * FROM dim_players""").pl()
        ...
        agg_player_view = players.join(player_aggregated_stats, on="player_id", how="left")
...
def filter_players(query_type: str = "polars", 
                   ...
                   g_xg_90_max: Optional[float] = None) -> pl.DataFrame:
    """
    ...
    if query_type == "polars":
        try:
            player_match_stats = conn.execute("""SELECT * FROM agg_player_stats_polars""").pl()
            ...
            result = player_match_stats if expr is None else player_match_stats.filter(expr).sort("player_id")
            return result 
        except Exception as e:
...
def get_team_summary(query_type: str = "polars", team_id: Optional[str] = None) -> pl.DataFrame: 
    ...
    if query_type == "polars":
        try:
            player_match_stats = conn.execute("""SELECT * FROM agg_player_stats_polars""").pl()
            teams = conn.execute("""SELECT team_id, league, name AS team_name FROM dim_teams""").pl()

            team_stats = (player_match_stats
                .join(
                ...
                .agg([
                    ...
                    pl.col('shot_accuracy').mean().round(2).alias('avg_shot_accuracy'),
                    pl.col('passes_attempted').sum().alias('total_passes_attempted'),
                    pl.col('passes_completed').sum().alias('total_passes_completed'),
                    pl.col('pass_accuracy').mean().round(2).alias('avg_pass_accuracy'),
                    ...
                    pl.col('duel_win_rate').mean().round(2).alias('avg_duel_win_rate'),
            ...
            if team_id is not None: 
                team_stats = team_stats.filter(pl.col("team_id") == team_id).drop('team_id')
