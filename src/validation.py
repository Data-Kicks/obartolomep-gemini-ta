...
handler = FileHandler(
    validation_path / f"validation_log_{timestamp}.log", encoding="utf-8"
)
...
class Player(BaseModel):
    ...
    def validate_birth_date_format(value):
        ...
        raise ValueError(
            f"Invalid date_of_birth format: {value}. Expected YYYY-MM-DD or DD/MM/YYYY"
        )
    ...
    def validate_contract_until_date_format(value):
        ...
        raise ValueError(
            f"Invalid contract_until format: {value}. Expected YYYY-MM-DD or DD/MM/YYYY"
        )
    ...
    def validate_match_date(value):
        ...
        for format in formats:
            try:
                ...
                continue
        raise ValueError(
            f"Invalid match_date format: {value}. Expected YYYY-MM-DD or DD/MM/YYYY"
        )
...
class PlayerMatchStats(BaseModel):
    ...
    def validate_shots_on_target(self):
        ...
        if self.shots_on_target > self.shots:
            raise ValueError(
                f"Shots on target ({self.shots_on_target}) cannot be greater than total shots ({self.shots})"
            )
        return self
    ...
    def validate_passes_completed(self):
        ...
        if self.passes_completed > self.passes_attempted:
            raise ValueError(
                f"Passes completed ({self.passes_completed}) cannot be greater than passes attempted ({self.passes_attempted})"
            )
        return self
    ...
    def validate_xg_logic(self):
        ...
        if self.xg > self.shots:
            raise ValueError(
                f"xG ({self.xg}) cannot be greater than total shots ({self.shots})"
            )
        return self
...
class MatchEvent(BaseModel):
    ...
    def validate_pass_destination(self):
        """Validate if pass have destination."""
    ...
    def validate_recipient_id(self):
        """Validate if pass has a recipient."""
    ...
    def validate_body_part(self):
        """Validate if a pass or shot has a body part."""
...
def validate_teams(teams_df: pl.DataFrame) -> Dict[str, Any]:
    ...
    duplicates = (
        teams_df.group_by("team_id").agg(count=pl.len()).filter(pl.col("count") > 1)
    )
    if len(duplicates) > 0:
        results["warnings"].append(
            f"Found {len(duplicates)} duplicate team ids: {duplicates['team_id'].to_list()}"
        )
    ...
    for i, row in enumerate(teams_df.to_dicts()):
        try:
            ...
            results["errors"].append(
                {"row": i, "team_id": row.get("team_id"), "errors": e.errors()}
            )
            logger.debug(f"Team validation error row {i}: {e.errors()}")

    logger.info(
        "Teams data validation finished with %d valid rows and %d invalid rows.",
        results["valid_rows"],
...
def validate_players(players_df: pl.DataFrame, team_id_list: list) -> Dict[str, Any]:
    ...
    orp_players = players_df.filter(pl.col("team_id").is_in(team_id_list).not_())
    if len(orp_players) > 0:
        results["warnings"].append(
            f"Found {len(orp_players)} orphaned team ids: {orp_players['team_id'].to_list()}"
        )

    # Check for duplicate player ids
    duplicates = (
        players_df.group_by("player_id").agg(count=pl.len()).filter(pl.col("count") > 1)
    )
    if len(duplicates) > 0:
        results["warnings"].append(
            f"Found {len(duplicates)} duplicate player ids: {duplicates['player_id'].to_list()}"
        )
    ...
    if "position" in players_df.columns:
        ...
        results["warnings"].append(
            f"Found {len(unique_positions)} different position labels: {unique_positions}"
        )
    ...
    for i, row in enumerate(players_df.to_dicts()):
        try:
            ...
            results["invalid_rows"] += 1
            results["errors"].append(
                {"row": i, "player_id": row.get("player_id"), "errors": e.errors()}
            )

    logger.info(
        "Players data validation finished with %d valid rows and %d invalid rows.",
        results["valid_rows"],
        results["invalid_rows"],
    )
...
def validate_matches(matches_df: pl.DataFrame) -> Dict[str, Any]:
    ...
    duplicates = (
        matches_df.group_by("match_id").agg(count=pl.len()).filter(pl.col("count") > 1)
    )
    if len(duplicates) > 0:
        results["warnings"].append(
            f"Found {len(duplicates)} duplicate match ids: {duplicates['match_id'].to_list()}"
        )
    ...
    for i, row in enumerate(matches_df.to_dicts()):
        try:
            ...
            results["invalid_rows"] += 1
            results["errors"].append(
                {"row": i, "match_id": row.get("match_id"), "errors": e.errors()}
            )

    logger.info(
        "Matches data validation finished with %d valid rows and %d invalid rows.",
        results["valid_rows"],
        results["invalid_rows"],
    )
...
def validate_player_match_stats(
    stats_df: pl.DataFrame, player_id_list: list, match_id_list: list
) -> Dict[str, Any]:
    """
    ...
    orp_players = stats_df.filter(pl.col("player_id").is_in(player_id_list).not_())
    if len(orp_players) > 0:
        results["warnings"].append(
            f"Found {len(orp_players)} orphaned player ids: {orp_players['player_id'].to_list()}"
        )

    orp_matches = stats_df.filter(pl.col("match_id").is_in(match_id_list).not_())
    if len(orp_matches) > 0:
        results["warnings"].append(
            f"Found {len(orp_matches)} orphaned match ids: {orp_matches['match_id'].to_list()}"
        )

    # Check for duplicate event_ids
    if ("player_id" in stats_df.columns) & ("match_id" in stats_df.columns):
        duplicates = (
            stats_df.select(["player_id", "match_id"])
            .group_by(["player_id", "match_id"])
            .agg(count=pl.len())
            .filter(pl.col("count") > 1)
        )
        if len(duplicates) > 0:
            duplicates = duplicates.with_columns(
                pl.concat_str(
                    [pl.col("player_id"), pl.col("match_id")], separator="-"
                ).alias("player_match_id")
            )
            results["warnings"].append(
                f"Found {len(duplicates)} duplicate player_match ids: {duplicates['player_match_id'].to_list()}"
            )
    ...
    logger.info(
        "Player match stats data validation finished with %d valid rows and %d invalid rows.",
        results["valid_rows"],
        results["invalid_rows"],
    )
...
def validate_match_events(
    ...
    orp_matches = events_df.filter(pl.col("match_id").is_in(match_id_list).not_())
    if len(orp_matches) > 0:
        results["warnings"].append(
            f"Found {len(orp_matches)} orphaned match ids: {orp_matches['match_id'].to_list()}"
        )

    orp_teams = events_df.filter(pl.col("team_id").is_in(team_id_list).not_())
    if len(orp_teams) > 0:
        results["warnings"].append(
            f"Found {len(orp_teams)} orphaned team ids: {orp_teams['team_id'].to_list()}"
        )

    orp_players = events_df.filter(pl.col("player_id").is_in(player_id_list).not_())
    if len(orp_players) > 0:
        results["warnings"].append(
            f"Found {len(orp_players)} orphaned player ids: {orp_players['player_id'].to_list()}"
        )
    ...
    if "event_id" in events_df.columns:
        ...
        if len(duplicates) > 0:
            results["warnings"].append(
                f"Found {len(duplicates)} duplicate event ids: {duplicates['event_id'].to_list()}"
            )
    ...
    for i, row in enumerate(events_df.to_dicts()):
        try:
            ...
            results["invalid_rows"] += 1
            results["errors"].append(
                {"row": i, "event_id": row.get("event_id"), "errors": str(e)}
            )

    logger.info(
        "Match events data validation finished with %d valid rows and %d invalid rows.",
        results["valid_rows"],
        results["invalid_rows"],
    )
...
def load_and_validate_data(datasets: Dict[str, pl.LazyFrame]):
    """
    ...
    if datasets["players"].count().collect().shape[1] > 0:
        validation_results["players"] = validate_players(
            datasets["players"].collect(), datasets["teams"].select("team_id").collect()
        )
    else:
    ...
    if datasets["player_match_stats"].count().collect().shape[1] > 0:
        ...
        logger.info(
            "Player match stats dataset is empty. Skipping player match stats validation."
        )
    ...
    for dataset_name in datasets:
        ...
        if results.get("errors"):
            ...
            for error in results["errors"]:
                if error.get("errors")[0]["type"] == "value_error":
                    lines.append(
                        f"    - {error.get('row')}: {error.get('errors')[0]['msg']}"
                    )
                else:
                    lines.append(
                        f"    - {error.get('row')}: {str(error.get('errors')[0]['loc']).replace('(', '').replace(')', '')} {error.get('errors')[0]['msg']}"
                    )
        ...
        if results.get("null_counts"):
            ...
            for col, counts in results["null_counts"].items():
                lines.append(
                    f"    - {col}: {counts['null_count']} ({counts['null_percentage']:.1f}%)"
                )
        lines.append("")
...
def main():
    ...
    try:
        ...
        results = load_and_validate_data(datasets)
        create_validation_report(results)
        logger.info(
            "Data validation step finished! See validation report for more information."
        )
    except Exception as e:
